...
export class Service {
  ...
  deleteMovie(id: string): Observable<User> {
    console.log('in delete service');
    return this._http.delete<User>(`api/bikes/${id}`);
  }
  ...
  //Movie
  createMovie(movie:User):Observable<User>{
    console.log('service create movie', movie);
    return this._http.post<User>('api/users', movie)
  }

  //get all the movies
  getMovies():Observable<User[]>{
    console.log('service getMovies');
    return this._http.get<User[]>('api/movies')
  }

  //get a single movie
  getMovie(id:String):Observable<User>{
    console.log('service getMovie');
    return this._http.get<User>(`api/movie/${id}`)
  }

  //add movieid onto the new review
  // editReviewAddMovieId(id:String):Observable<Bike>{
  //   console.log('service editReviewAddMovieId')
  //   return this._http.put<Bike>(`api/movie/${id}`,)
  // }

  //USER FOR EDITING THE BIKE
  editMovieAddReviewId(bike: Bike):Observable<Bike>{
    console.log('service editMovie');
    return this._http.put<Bike>(`api/bikes/${bike._id}`, bike)
  }
    

  //Revieww with movie
  createReview(review:User):Observable<Bike>{
    console.log('Service create review, review')
    return this._http.post<Bike>('api/bikes', review);
  }

  //create just a review
  createReviewOnly(review:Bike):Observable<Bike>{
    console.log('Service create review only')
    return this._http.post<Bike>('api/bikeOnly',review)
  }

  //attach review to movie
  editMovieReviews(movie:User, review:Bike){
    return this._http.put<User>(`api/addreviewstart/${movie._id}`, review)
  }
